...
public class ModelResource extends AbstractModelResource{
    ...
    public ModelRepresentation updateModel(@PathVariable String modelId, @RequestBody ModelRepresentation updatedModel){
        ...
        if(modelKeyInfo.isKeyAlreadyExists()){
            throw new BusinessException("Model with provided key already exists " + updatedModel.getKey());
        }
        ...
        try{
            ...
            throw new BusinessException("Model cannot be updated: " + modelId);
        }
    }
    ...
    public void deleteModel(@PathVariable String modelId, @RequestParam(required = false) Boolean cascade, @RequestParam(required = false) Boolean deleteRuntimeApp){
        ...
        try{
            String currentUserId = ((User) sessionDeal.getNowUserFront()).getUserId();
            ...
            throw new BusinessException("Model cannot be deleted: " + modelId);
        }
    }
     ...
     */
    @RequestMapping(value = "/rest/models/{modelId}/editor/json", method = RequestMethod.POST, produces = "application/json")
    //    public ModelRepresentation saveModel(@PathVariable String modelId, @RequestBody MultiValueMap<String, String> values){
    @ResponseBody
    public ModelRepresentation saveModel(@PathVariable String modelId, @RequestParam MultiValueMap<String, String> values){

        // Validation: see if there was another update in the meantime
        long lastUpdated = -1L;
        String lastUpdatedString = values.getFirst("lastUpdated");
        if(lastUpdatedString == null){
            throw new BusinessException("Missing lastUpdated date");
        }
        try{
            ...
            throw new BusinessException("Invalid lastUpdated date: '" + lastUpdatedString + "'");
        }

        Model model = modelService.getModel(modelId);
        User currentUser = (User) sessionDeal.getNowUserFront();
        boolean currentUserIsOwner = model.getLastUpdatedBy().equals(currentUser.getUserId());
        ...
        if(model.getLastUpdated().getTime() != lastUpdated){
            ...
            if(RESOLVE_ACTION_SAVE_AS.equals(resolveAction)){
                ...
                if(currentUserIsOwner && "true".equals(isNewVersionString)){
                    ...
                    //                    throw exception;
                    throw new BusinessException("Process model was updated in the meantime");
                }
            }
    ...
    protected ModelRepresentation updateModel(Model model, MultiValueMap<String, String> values, boolean forceNewVersion){
        ...
        if(modelKeyInfo.isKeyAlreadyExists()){
            throw new BusinessException("Model with provided key already exists " + key);
        }
        ...
        try{
            model = modelService.saveModel(model.getId(), name, key, description, json, newVersion, newVersionComment, (User) sessionDeal.getNowUserFront());
            return new ModelRepresentation(model);

        }catch(Exception e){
            log.error("Error saving model " + model.getId(), e);
            throw new BusinessException("Process model could not be saved " + model.getId());
        }
    }
    ...
    protected ModelRepresentation createNewModel(String name, String description, Integer modelType, String editorJson){
        ...
        Model newModel = modelService.createModel(model, editorJson, (User) sessionDeal.getNowUserFront());
